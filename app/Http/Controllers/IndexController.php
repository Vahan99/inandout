...
//    }
    ...
    public function tours(Request $request, $slug)
    {
        ...
        if(isset($request->range_val) && $request->range_val != 0) {
            ...
            foreach($tours->get() as $tour){
                ...
                if(isset($data)){
                    foreach ($data as $d){
                        dump(currency($d['price'], $request->currency, currency()->getUserCurrency()), "------", $request->range_val, currency($d['price'], $request->currency, currency()->getUserCurrency()) <= $request->range_val);
    ...
    public function sendContact(Request $request)
    {
        ...
        {
            $message->from('inandouttour@gmail.com', 'inandouttour@gmail.com');
            $message->to('inandouttour@gmail.com', 'inandouttour@gmail.com');
    ...
    public function sendTourMail(Request $request)
    {
        ...
        Mail::send('site.mails.index', ['data' => $data], function ($message) use ($data)
        {
            ...
            $message->bcc('inandouttour@gmail.com', 'inandouttour@gmail.com');
            $message->cc('inandouttour@gmail.com', 'inandouttour@gmail.com');
            $message->subject("Inandout Travel Agency");
        });
